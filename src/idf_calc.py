...
def calculate_vacation(total_days, has_children, is_combat):
    if total_days < 60:
        return 0
    base_vacation = 3500 if is_combat else 1500
    additional_for_children = 1000 if is_combat else 500
    return base_vacation + additional_for_children if has_children else base_vacation
...
def is_one_range_more_than_5_days(date_ranges):
    return any(total_days_in_range(date_range['startDate'], date_range['endDate']) >= 5 for date_range in date_ranges)
...
def calculate_compensation(inputs):
    date_ranges = [{'startDate': datetime.fromisoformat(date_range['startDate']),
                    'endDate': datetime.fromisoformat(date_range['endDate'])} for date_range in inputs['dateRanges']]
    ...
    result = special_grant_calculation(service_before, days_in_war, inputs['isDaysStraight'] or is_days_straight_in_war)
    total_per_month = calculate_monthly_compensation(inputs['isCombat'], max(days_in_war, 0))
    ...
    total_from_children = calculate_children_compensation(inputs['isCombat'], days_war_in_2023) if inputs['hasChildren'] else 0
    total_vacation = calculate_vacation(days_in_war, inputs['hasChildren'], inputs['isCombat'])
    total_special_children = SPECIAL_NEEDS_COMPENSATION if inputs['hasChildrenSpecial'] else 0
